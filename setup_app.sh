...
# =============================================================================
# Dr. Mays Nutrition System - Complete Ubuntu Setup Script
# =============================================================================
# This script installs and configures the complete Dr. Mays Nutrition System
# on Ubuntu server with all required packages and services.
# =============================================================================
...
# =============================================================================
print_header "Dr. Mays Nutrition System - Ubuntu Setup"
print_status "Starting complete system installation..."
...
# =============================================================================
# STEP 3: DATABASE SETUP (PRESERVE EXISTING)
# =============================================================================
print_step "Step 3: Database Setup (Preserving Existing Configuration)"

print_status "Starting PostgreSQL service..."
sudo systemctl start postgresql
sudo systemctl enable postgresql

print_status "Checking existing database configuration..."
# Note: We will NOT modify existing database credentials
# The application will use the existing database with current username/password

print_success "Database service ready (existing configuration preserved)"
...
# Clone or setup project (assuming project files are already here)
if [ ! -d "Nutrition-Mays" ]; then
    print_status "Creating project structure..."
    mkdir -p Nutrition-Mays
    cd Nutrition-Mays
else
    cd Nutrition-Mays
fi
...
DATABASE_URL=postgresql://username:password@localhost:5432/drmays_db
ALLOWED_HOSTS=158.178.143.50,mayslife.uk,localhost,127.0.0.1
EOF
    print_warning "Please update .env file with your actual database credentials"
fi
...
if [ -f "dr_mays_nutrition/settings.py" ]; then
    cp dr_mays_nutrition/settings.py dr_mays_nutrition/settings.py.backup
fi

# Add production settings to settings.py
...
# Static files
STATIC_URL = '/static/'
STATIC_ROOT = '/var/www/drmays/staticfiles'
MEDIA_URL = '/media/'
MEDIA_ROOT = '/var/www/drmays/media'
...
sudo mkdir -p /var/www/drmays/media
sudo chown -R www-data:www-data /var/www/drmays/media
sudo chmod -R 755 /var/www/drmays/media
...
preload_app = True
accesslog = "/var/log/drmays/access.log"
errorlog = "/var/log/drmays/error.log"
loglevel = "info"
EOF
...
sudo chown www-data:www-data /var/log/drmays
...
User=www-data
Group=www-data
WorkingDirectory=$PROJECT_DIR/Nutrition-Mays
Environment=PATH=$PROJECT_DIR/Nutrition-Mays/venv/bin:/usr/local/bin:/usr/bin:/bin
Environment=DJANGO_SETTINGS_MODULE=dr_mays_nutrition.settings
ExecStartPre=/bin/sleep 10
ExecStart=$PROJECT_DIR/Nutrition-Mays/venv/bin/gunicorn --config $PROJECT_DIR/Nutrition-Mays/gunicorn.conf.py dr_mays_nutrition.wsgi:application
...
server {
    listen 80;
    server_name 158.178.143.50 mayslife.uk www.mayslife.uk;
...
sudo chmod +x $PROJECT_DIR/Nutrition-Mays/manage.py
...
# =============================================================================
# STEP 16: SSL CERTIFICATE (OPTIONAL)
# =============================================================================
print_step "Step 16: SSL Certificate Setup (Optional)"
...
echo -e "  • Database: ${GREEN}PostgreSQL (existing configuration preserved)${NC}"
...
echo -e "  • Project: ${YELLOW}$PROJECT_DIR/Nutrition-Mays${NC}"
echo -e "  • Static Files: ${YELLOW}/var/www/drmays/staticfiles${NC}"
echo -e "  • Media Files: ${YELLOW}/var/www/drmays/media${NC}"
echo -e "  • Logs: ${YELLOW}/var/log/drmays${NC}"
echo ""
...
echo -e "${YELLOW}⚠️  Please update your .env file with actual database credentials if needed.${NC}"
