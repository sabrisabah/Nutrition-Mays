import React, { useState, useEffect } from 'react'

const DietPlansSystem = ({ patientProfile, nutritionData, onDietPlanSelected }) => {
  const [selectedDietPlan, setSelectedDietPlan] = useState(null)
  const [customizedPlan, setCustomizedPlan] = useState(null)
  const [isLoading, setIsLoading] = useState(false)
  const [showFullDetails, setShowFullDetails] = useState(false)

  // تعريف الأنظمة الغذائية المتكاملة
  const dietPlans = {
    keto: {
      id: 'keto',
      name: 'نظام الكيتو دايت',
      description: 'نظام غذائي عالي الدهون ومنخفض الكربوهيدرات لحرق الدهون',
      icon: '🥑',
      color: 'success',
      macroDistribution: {
        carbs: 5,    // 5% من السعرات
        protein: 20, // 20% من السعرات
        fat: 75      // 75% من السعرات
      },
      dailyMeals: {
        breakfast: [
          'بيض مقلي بالزبدة (2 بيضة)',
          'أفوكادو (نصف حبة)',
          'لحم بقري (2 شريحة)',
          'قهوة بالزبدة'
        ],
        lunch: [
          'سلطة خضراء مع زيت الزيتون',
          'سمك سلمون مشوي (150 جم)',
          'بروكلي مطهو بالزبدة',
          'جبنة شيدر (30 جم)'
        ],
        dinner: [
          'لحم بقري مشوي (200 جم)',
          'مرق سبانخ',
          'فطر مشوي',
          'زيتون أسود'
        ],
        snacks: [
          'جوز (15 حبة)',
          'جبنة كاملة الدسم (30 جم)',
          'زيتون (10 حبات)'
        ]
      },
      allowedFoods: [
        'اللحوم الحمراء والدواجن',
        'الأسماك الدهنية',
        'البيض',
        'الأفوكادو',
        'المكسرات والبذور',
        'الزيوت الصحية',
        'الخضروات الورقية',
        'الجبنة كاملة الدسم'
      ],
      restrictedFoods: [
        'الحبوب والنشويات',
        'الفواكه عالية السكر',
        'البقوليات',
        'السكريات والحلويات',
        'المشروبات المحلاة'
      ],
      benefits: [
        'فقدان سريع للوزن',
        'تحسين حساسية الأنسولين',
        'زيادة الطاقة والتركيز',
        'تقليل الالتهابات'
      ],
      considerations: [
        'يحتاج فترة تكيف 2-4 أسابيع',
        'قد يسبب إمساك في البداية',
        'يحتاج شرب ماء أكثر',
        'يُنصح بمراقبة الكيتونات'
      ]
    },
    balanced: {
      id: 'balanced',
      name: 'النظام المتوازن',
      description: 'نظام غذائي متوازن يحتوي على جميع العناصر الغذائية',
      icon: '⚖️',
      color: 'primary',
      macroDistribution: {
        carbs: 50,   // 50% من السعرات
        protein: 25, // 25% من السعرات
        fat: 25      // 25% من السعرات
      },
      dailyMeals: {
        breakfast: [
          'شوفان مع الحليب (كوب)',
          'موز (حبة متوسطة)',
          'جوز (10 حبات)',
          'عسل (ملعقة صغيرة)'
        ],
        lunch: [
          'أرز بني (كوب)',
          'دجاج مشوي (150 جم)',
          'خضروات مشكلة',
          'سلطة خضراء'
        ],
        dinner: [
          'سمك مشوي (150 جم)',
          'بطاطا حلوة مشوية',
          'بروكلي مطهو',
          'سلطة خضراء'
        ],
        snacks: [
          'تفاح (حبة)',
          'زبدة الفول السوداني (ملعقة)',
          'جبنة قليلة الدسم (30 جم)'
        ]
      },
      allowedFoods: [
        'جميع أنواع الحبوب الكاملة',
        'الفواكه والخضروات',
        'البروتينات الخالية من الدهون',
        'الدهون الصحية',
        'منتجات الألبان قليلة الدسم',
        'المكسرات والبذور'
      ],
      restrictedFoods: [
        'الأطعمة المصنعة',
        'السكريات المضافة',
        'الدهون المتحولة',
        'المشروبات المحلاة'
      ],
      benefits: [
        'توازن غذائي مثالي',
        'سهل المتابعة',
        'مناسب لجميع الأعمار',
        'يدعم الصحة العامة'
      ],
      considerations: [
        'يحتاج تخطيط وجبات',
        'مرونة في الاختيارات',
        'مناسب للرياضيين',
        'يدعم فقدان الوزن التدريجي'
      ]
    },
    weightGain: {
      id: 'weightGain',
      name: 'نظام زيادة الوزن',
      description: 'نظام غذائي عالي السعرات لزيادة الوزن بطريقة صحية',
      icon: '📈',
      color: 'warning',
      macroDistribution: {
        carbs: 45,   // 45% من السعرات
        protein: 25, // 25% من السعرات
        fat: 30      // 30% من السعرات
      },
      dailyMeals: {
        breakfast: [
          'عجة بالجبن (3 بيضات)',
          'خبز كامل (2 شريحة)',
          'أفوكادو (حبة كاملة)',
          'حليب كامل الدسم (كوب)'
        ],
        lunch: [
          'أرز أبيض (كوب ونصف)',
          'لحم بقري (200 جم)',
          'بطاطا مقلية',
          'سلطة مع زيت الزيتون'
        ],
        dinner: [
          'معكرونة باللحم (كوب ونصف)',
          'دجاج مشوي (200 جم)',
          'خضروات مطهوة',
          'جبنة بارميزان'
        ],
        snacks: [
          'مكسرات مختلطة (حفنة)',
          'موز مع زبدة الفول السوداني',
          'حليب بالشوكولاتة',
          'جبنة كاملة الدسم'
        ]
      },
      allowedFoods: [
        'الحبوب الكاملة والبيضاء',
        'البروتينات عالية السعرات',
        'الدهون الصحية',
        'الفواكه المجففة',
        'المكسرات والبذور',
        'منتجات الألبان كاملة الدسم'
      ],
      restrictedFoods: [
        'الأطعمة منخفضة السعرات',
        'المشروبات الدايت',
        'الخضروات الورقية فقط',
        'الأطعمة قليلة الدهون'
      ],
      benefits: [
        'زيادة الوزن الصحية',
        'بناء العضلات',
        'تحسين القوة البدنية',
        'زيادة الطاقة'
      ],
      considerations: [
        'يحتاج تناول وجبات متكررة',
        'مراقبة زيادة الوزن',
        'ممارسة الرياضة مهمة',
        'تجنب الدهون غير الصحية'
      ]
    },
    pregnancy: {
      id: 'pregnancy',
      name: 'نظام الحامل',
      description: 'نظام غذائي مخصص للحوامل مع التركيز على العناصر الغذائية المهمة',
      icon: '🤱',
      color: 'info',
      macroDistribution: {
        carbs: 50,   // 50% من السعرات
        protein: 20, // 20% من السعرات
        fat: 30      // 30% من السعرات
      },
      dailyMeals: {
        breakfast: [
          'شوفان مع الحليب (كوب)',
          'موز (حبة)',
          'جوز (10 حبات)',
          'عصير برتقال طبيعي'
        ],
        lunch: [
          'أرز بني (كوب)',
          'سمك سلمون (150 جم)',
          'خضروات مشكلة',
          'سلطة خضراء'
        ],
        dinner: [
          'دجاج مشوي (150 جم)',
          'بطاطا حلوة مشوية',
          'بروكلي مطهو',
          'سلطة خضراء'
        ],
        snacks: [
          'تفاح (حبة)',
          'جبنة قليلة الدسم (30 جم)',
          'زبدة الفول السوداني (ملعقة)',
          'حليب (كوب)'
        ]
      },
      allowedFoods: [
        'البروتينات الخالية من الدهون',
        'الحبوب الكاملة',
        'الفواكه والخضروات',
        'منتجات الألبان',
        'الأسماك الدهنية',
        'المكسرات والبذور'
      ],
      restrictedFoods: [
        'الأسماك عالية الزئبق',
        'اللحوم النيئة',
        'الأجبان الطرية',
        'الكافيين الزائد',
        'الكحول'
      ],
      benefits: [
        'دعم نمو الجنين',
        'منع فقر الدم',
        'تقليل الغثيان',
        'دعم صحة الأم'
      ],
      considerations: [
        'يحتاج حمض الفوليك',
        'الحديد مهم جداً',
        'تجنب الأطعمة الخطيرة',
        'شرب ماء كافي'
      ]
    },
    breastfeeding: {
      id: 'breastfeeding',
      name: 'نظام المرضع',
      description: 'نظام غذائي مخصص للمرضعات مع زيادة السعرات والعناصر الغذائية',
      icon: '🍼',
      color: 'info',
      macroDistribution: {
        carbs: 50,   // 50% من السعرات
        protein: 20, // 20% من السعرات
        fat: 30      // 30% من السعرات
      },
      dailyMeals: {
        breakfast: [
          'شوفان مع الحليب (كوب ونصف)',
          'موز (حبة)',
          'جوز (15 حبة)',
          'عسل (ملعقة كبيرة)'
        ],
        lunch: [
          'أرز بني (كوب ونصف)',
          'دجاج مشوي (200 جم)',
          'خضروات مشكلة',
          'سلطة خضراء'
        ],
        dinner: [
          'سمك مشوي (200 جم)',
          'بطاطا حلوة مشوية',
          'بروكلي مطهو',
          'سلطة خضراء'
        ],
        snacks: [
          'تفاح (حبة)',
          'جبنة قليلة الدسم (40 جم)',
          'زبدة الفول السوداني (ملعقة كبيرة)',
          'حليب (كوب ونصف)'
        ]
      },
      allowedFoods: [
        'البروتينات عالية الجودة',
        'الحبوب الكاملة',
        'الفواكه والخضروات',
        'منتجات الألبان',
        'الأسماك الدهنية',
        'المكسرات والبذور'
      ],
      restrictedFoods: [
        'الكافيين الزائد',
        'الأسماك عالية الزئبق',
        'الأطعمة الحارة جداً',
        'الكحول'
      ],
      benefits: [
        'دعم إنتاج الحليب',
        'تغذية الطفل',
        'استعادة صحة الأم',
        'زيادة الطاقة'
      ],
      considerations: [
        'زيادة السعرات 500-600 يومياً',
        'شرب ماء كافي',
        'تجنب الأطعمة المسببة للحساسية',
        'مراقبة وزن الطفل'
      ]
    },
    diabetes: {
      id: 'diabetes',
      name: 'نظام السكري',
      description: 'نظام غذائي مخصص لمرضى السكري مع التحكم في الكربوهيدرات',
      icon: '🩺',
      color: 'danger',
      macroDistribution: {
        carbs: 40,   // 40% من السعرات
        protein: 30, // 30% من السعرات
        fat: 30      // 30% من السعرات
      },
      dailyMeals: {
        breakfast: [
          'شوفان مع الحليب قليل الدسم',
          'توت (نصف كوب)',
          'جوز (10 حبات)',
          'قهوة بدون سكر'
        ],
        lunch: [
          'أرز بني (نصف كوب)',
          'دجاج مشوي (150 جم)',
          'خضروات مشكلة',
          'سلطة خضراء'
        ],
        dinner: [
          'سمك مشوي (150 جم)',
          'خضروات مطهوة',
          'سلطة خضراء',
          'زيت الزيتون'
        ],
        snacks: [
          'تفاح (حبة صغيرة)',
          'جبنة قليلة الدسم (30 جم)',
          'مكسرات (حفنة صغيرة)'
        ]
      },
      allowedFoods: [
        'الخضروات غير النشوية',
        'البروتينات الخالية من الدهون',
        'الحبوب الكاملة',
        'الفواكه قليلة السكر',
        'الدهون الصحية',
        'منتجات الألبان قليلة الدسم'
      ],
      restrictedFoods: [
        'السكريات المضافة',
        'الحبوب المكررة',
        'الفواكه عالية السكر',
        'المشروبات المحلاة',
        'الأطعمة المصنعة'
      ],
      benefits: [
        'تحكم في سكر الدم',
        'تقليل مقاومة الأنسولين',
        'فقدان الوزن',
        'تحسين صحة القلب'
      ],
      considerations: [
        'مراقبة سكر الدم',
        'توزيع الكربوهيدرات',
        'تجنب السكريات البسيطة',
        'ممارسة الرياضة مهمة'
      ]
    },
    weightMaintenance: {
      id: 'weightMaintenance',
      name: 'نظام تثبيت الوزن',
      description: 'نظام غذائي للحفاظ على الوزن الحالي مع التوازن الغذائي',
      icon: '⚖️',
      color: 'secondary',
      macroDistribution: {
        carbs: 45,   // 45% من السعرات
        protein: 25, // 25% من السعرات
        fat: 30      // 30% من السعرات
      },
      dailyMeals: {
        breakfast: [
          'شوفان مع الحليب (كوب)',
          'موز (حبة متوسطة)',
          'جوز (10 حبات)',
          'عسل (ملعقة صغيرة)'
        ],
        lunch: [
          'أرز بني (كوب)',
          'دجاج مشوي (150 جم)',
          'خضروات مشكلة',
          'سلطة خضراء'
        ],
        dinner: [
          'سمك مشوي (150 جم)',
          'بطاطا حلوة مشوية',
          'بروكلي مطهو',
          'سلطة خضراء'
        ],
        snacks: [
          'تفاح (حبة)',
          'جبنة قليلة الدسم (30 جم)',
          'زبدة الفول السوداني (ملعقة)'
        ]
      },
      allowedFoods: [
        'جميع أنواع الحبوب الكاملة',
        'الفواكه والخضروات',
        'البروتينات الخالية من الدهون',
        'الدهون الصحية',
        'منتجات الألبان قليلة الدسم',
        'المكسرات والبذور'
      ],
      restrictedFoods: [
        'الأطعمة عالية السعرات',
        'السكريات المضافة',
        'الدهون المتحولة',
        'المشروبات المحلاة'
      ],
      benefits: [
        'الحفاظ على الوزن',
        'توازن غذائي',
        'صحة عامة جيدة',
        'طاقة مستمرة'
      ],
      considerations: [
        'مراقبة الوزن أسبوعياً',
        'تعديل السعرات حسب النشاط',
        'ممارسة الرياضة',
        'تجنب الإفراط في الطعام'
      ]
    }
  }

  // حساب التوزيع المخصص للعناصر الغذائية
  const calculateCustomizedMacros = (dietPlan) => {
    if (!nutritionData || !dietPlan) return null

    const { targetCalories } = nutritionData
    const { macroDistribution } = dietPlan

    const carbs = Math.round((targetCalories * macroDistribution.carbs / 100) / 4)
    const protein = Math.round((targetCalories * macroDistribution.protein / 100) / 4)
    const fat = Math.round((targetCalories * macroDistribution.fat / 100) / 9)

    return {
      carbs,
      protein,
      fat,
      calories: targetCalories
    }
  }

  // تحديث الخطة المخصصة عند تغيير النظام
  useEffect(() => {
    if (selectedDietPlan) {
      const customized = calculateCustomizedMacros(dietPlans[selectedDietPlan])
      setCustomizedPlan(customized)
      
      if (onDietPlanSelected) {
        onDietPlanSelected({
          plan: dietPlans[selectedDietPlan],
          customizedMacros: customized
        })
      }
    }
  }, [selectedDietPlan, nutritionData])

  const handleDietPlanSelect = (planId) => {
    setIsLoading(true)
    setSelectedDietPlan(planId)
    setShowFullDetails(false) // إعادة تعيين عرض التفاصيل
    
    // محاكاة تحميل سريع لتحسين تجربة المستخدم
    setTimeout(() => {
      setIsLoading(false)
    }, 300)
  }

  return (
    <div className="card">
      <div className="card-header">
        <h5 className="mb-0">
          <i className="fas fa-utensils text-primary me-2"></i>
          الأنظمة الغذائية المتكاملة
        </h5>
        <p className="text-muted mb-0">اختر النظام الغذائي المناسب للمريض</p>
      </div>
      <div className="card-body">
        {/* تحذير إذا لم تكن بيانات التغذية متوفرة */}
        {!nutritionData && (
          <div className="alert alert-warning mb-4">
            <i className="fas fa-exclamation-triangle me-2"></i>
            <strong>تحذير:</strong> لم يتم حساب بيانات التغذية للمريض بعد. 
            يرجى التأكد من إدخال الوزن والطول والهدف ومستوى النشاط في الملف الشخصي.
          </div>
        )}

        {/* عرض الأنظمة الغذائية */}
        <div className="row mb-4">
          {Object.values(dietPlans).map((plan) => (
            <div key={plan.id} className="col-md-6 col-lg-4 mb-3">
              <div 
                className={`card h-100 cursor-pointer border-${selectedDietPlan === plan.id ? plan.color : 'light'}`}
                onClick={() => handleDietPlanSelect(plan.id)}
                style={{ 
                  cursor: 'pointer',
                  borderWidth: selectedDietPlan === plan.id ? '2px' : '1px'
                }}
              >
                <div className="card-body text-center">
                  <div className="fs-1 mb-2">{plan.icon}</div>
                  <h6 className={`text-${plan.color} mb-2`}>{plan.name}</h6>
                  <p className="text-muted small mb-3">{plan.description}</p>
                  <div className="d-flex justify-content-center">
                    <span className={`badge bg-${plan.color} bg-opacity-10 text-${plan.color}`}>
                      {plan.macroDistribution.carbs}% كربوهيدرات
                    </span>
                  </div>
                </div>
              </div>
            </div>
          ))}
        </div>

        {/* مؤشر التحميل */}
        {isLoading && (
          <div className="text-center py-4">
            <div className="spinner-border text-primary" role="status">
              <span className="visually-hidden">جاري التحميل...</span>
            </div>
            <p className="text-muted mt-2">جاري تحميل تفاصيل النظام الغذائي...</p>
          </div>
        )}

        {/* عرض تفاصيل النظام المختار */}
        {selectedDietPlan && customizedPlan && !isLoading && (
          <div className="border rounded p-4 bg-light">
            <div className="row">
              <div className="col-md-6">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <h6 className="text-primary mb-0">
                    {dietPlans[selectedDietPlan].icon} {dietPlans[selectedDietPlan].name}
                  </h6>
                  <button 
                    className="btn btn-sm btn-outline-primary"
                    onClick={() => setShowFullDetails(!showFullDetails)}
                  >
                    {showFullDetails ? 'إخفاء التفاصيل' : 'عرض التفاصيل الكاملة'}
                  </button>
                </div>
                
                {/* التوزيع المخصص للعناصر الغذائية */}
                <div className="mb-3">
                  <h6 className="text-success">التوزيع المخصص للعناصر الغذائية:</h6>
                  <div className="row">
                    <div className="col-4">
                      <div className="text-center p-2 bg-primary bg-opacity-10 rounded">
                        <div className="fw-bold text-primary">{customizedPlan.carbs} جم</div>
                        <small className="text-muted">كربوهيدرات</small>
                      </div>
                    </div>
                    <div className="col-4">
                      <div className="text-center p-2 bg-warning bg-opacity-10 rounded">
                        <div className="fw-bold text-warning">{customizedPlan.protein} جم</div>
                        <small className="text-muted">بروتين</small>
                      </div>
                    </div>
                    <div className="col-4">
                      <div className="text-center p-2 bg-danger bg-opacity-10 rounded">
                        <div className="fw-bold text-danger">{customizedPlan.fat} جم</div>
                        <small className="text-muted">دهون</small>
                      </div>
                    </div>
                  </div>
                  <div className="text-center mt-2">
                    <span className="badge bg-success">
                      إجمالي السعرات: {customizedPlan.calories} سعرة
                    </span>
                  </div>
                </div>

                {/* الوجبات اليومية - عرض فقط عند الحاجة */}
                {showFullDetails && (
                  <div className="mb-3">
                    <h6 className="text-info">الوجبات اليومية المقترحة:</h6>
                    <div className="row">
                      <div className="col-6">
                        <div className="mb-2">
                          <strong className="text-primary">الإفطار:</strong>
                          <ul className="small mb-0">
                            {dietPlans[selectedDietPlan].dailyMeals.breakfast.map((item, index) => (
                              <li key={index}>{item}</li>
                            ))}
                          </ul>
                        </div>
                        <div className="mb-2">
                          <strong className="text-success">الغداء:</strong>
                          <ul className="small mb-0">
                            {dietPlans[selectedDietPlan].dailyMeals.lunch.map((item, index) => (
                              <li key={index}>{item}</li>
                            ))}
                          </ul>
                        </div>
                      </div>
                      <div className="col-6">
                        <div className="mb-2">
                          <strong className="text-warning">العشاء:</strong>
                          <ul className="small mb-0">
                            {dietPlans[selectedDietPlan].dailyMeals.dinner.map((item, index) => (
                              <li key={index}>{item}</li>
                            ))}
                          </ul>
                        </div>
                        <div className="mb-2">
                          <strong className="text-info">الوجبات الخفيفة:</strong>
                          <ul className="small mb-0">
                            {dietPlans[selectedDietPlan].dailyMeals.snacks.map((item, index) => (
                              <li key={index}>{item}</li>
                            ))}
                          </ul>
                        </div>
                      </div>
                    </div>
                  </div>
                )}
              </div>

              <div className="col-md-6">
                {/* الأطعمة المسموحة والممنوعة - عرض فقط عند الحاجة */}
                {showFullDetails && (
                  <div className="row">
                    <div className="col-6">
                      <h6 className="text-success">الأطعمة المسموحة:</h6>
                      <ul className="small">
                        {dietPlans[selectedDietPlan].allowedFoods.map((food, index) => (
                          <li key={index} className="text-success">{food}</li>
                        ))}
                      </ul>
                    </div>
                    <div className="col-6">
                      <h6 className="text-danger">الأطعمة الممنوعة:</h6>
                      <ul className="small">
                        {dietPlans[selectedDietPlan].restrictedFoods.map((food, index) => (
                          <li key={index} className="text-danger">{food}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
                )}

                {/* الفوائد والاعتبارات - عرض فقط عند الحاجة */}
                {showFullDetails && (
                  <div className="row mt-3">
                    <div className="col-6">
                      <h6 className="text-primary">الفوائد:</h6>
                      <ul className="small">
                        {dietPlans[selectedDietPlan].benefits.map((benefit, index) => (
                          <li key={index} className="text-primary">{benefit}</li>
                        ))}
                      </ul>
                    </div>
                    <div className="col-6">
                      <h6 className="text-warning">اعتبارات مهمة:</h6>
                      <ul className="small">
                        {dietPlans[selectedDietPlan].considerations.map((consideration, index) => (
                          <li key={index} className="text-warning">{consideration}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
                )}
              </div>
            </div>
          </div>
        )}
      </div>
    </div>
  )
}

export default DietPlansSystem
