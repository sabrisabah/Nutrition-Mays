...
# Fix Gunicorn and Services Script for Dr. Mays Nutrition Platform
# This script fixes gunicorn installation and systemd service configuration
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Fix Gunicorn and Services Script${NC}"
echo -e "${BLUE}========================================${NC}"
...
if sudo -u mays bash -c "cd $PROJECT_DIR && source venv/bin/activate && which gunicorn"; then
...
print_status "Testing gunicorn manually..."
sudo -u mays bash -c "cd $PROJECT_DIR && source venv/bin/activate && export DJANGO_SETTINGS_MODULE=dr_mays_nutrition.settings_production && timeout 10 gunicorn --workers 3 --bind 127.0.0.1:8000 --timeout 120 --keep-alive 2 --max-requests 1000 --max-requests-jitter 100 dr_mays_nutrition.wsgi:application" || true
...
echo -e "\n${BLUE}========================================${NC}"
echo -e "${GREEN}Gunicorn and Services Fix Complete!${NC}"
echo -e "${BLUE}========================================${NC}"
