...
# Fix Database Connection Script for Dr. Mays Nutrition Platform
# This script fixes database password authentication and updates configuration
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Fix Database Connection Script${NC}"
echo -e "${BLUE}========================================${NC}"
...
if [[ $EUID -ne 0 ]]; then
   print_error "This script must be run as root to fix database configuration."
   exit 1
fi
...
print_status "Generating new secure database password..."
NEW_PASSWORD=$(openssl rand -base64 32)
echo -e "${YELLOW}New database password: $NEW_PASSWORD${NC}"

# Update PostgreSQL user password
print_status "Updating PostgreSQL user password..."
sudo -u postgres psql << EOF
-- Drop and recreate user with new password
DROP USER IF EXISTS $DB_USER;
CREATE USER $DB_USER WITH PASSWORD '$NEW_PASSWORD' CREATEDB SUPERUSER;
...
# Update Django settings file with new password
print_status "Updating Django settings with new password..."
...
sudo -u mays bash -c "cd $PROJECT_DIR && source venv/bin/activate && export DJANGO_SETTINGS_MODULE=dr_mays_nutrition.settings_production && python manage.py check --database default"

if [ $? -eq 0 ]; then
    print_status "Django database connection test successful"
else
    print_error "Django database connection test failed"
    exit 1
fi
...
print_status "Database connection fix completed successfully!"

echo -e "\n${BLUE}========================================${NC}"
echo -e "${GREEN}Database Connection Fix Complete!${NC}"
echo -e "${BLUE}========================================${NC}"
...
print_status "Your Django application is now running with proper database connection!"
